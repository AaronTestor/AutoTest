...
<xsl:template name="pageHeader">
	...
	<table width="100%">
		<tr>
			<!-- 获取requestHeader数据 -->
			<xsl:variable name="URL" select="/httpSample/requestHeader" />
			<xsl:for-each select="$URL">
			</xsl:for-each>
			<!-- 从获取的URL中截取数据，第二代表起始位置，第三位代表长度 -->
		    <xsl:variable name="newURL" select="substring-after($URL,'Host:')" />
			<!-- 已换行符来截取，before代表换行符之前的数据 -->
			<xsl:variable name="remaining" select="substring($newURL,1,20)" /> 
			<td align="left">Date report: <xsl:value-of select="$dateReport" /></td>
			<td align="center">Host: <xsl:value-of select="substring($newURL,1,20)" /></td>
			<td align="right">Report: <a href="../../../Jmeter/apache-jmeter-2.13/ReportHtml/TestReport_test.html">Detail Result</a></td>
		</tr>
	</table>
	<hr size="1" />
</xsl:template>
	...
	<table align="center" class="details" border="0" cellpadding="5" cellspacing="2" width="95%">
		...
		<xsl:for-each select="/testResults/*[not(@lb = preceding::*/@lb)]"	>
			...
			<xsl:if test="$successCount='0'">
							<xsl:variable name="qpsTime" select="'0'"/>
						</xsl:if>
			<!-- <xsl:if select="$t = '0'">
			<xsl:variable name="qpsTime" select="0"/>
			</xsl:if> -->
		...
		<table align="center" class="details" border="0" cellpadding="5" cellspacing="2" width="95%">
				<tr valign="top">
					...
					<xsl:if test="$showData = 'y'">
						<th align="right">Response Data</th>
					</xsl:if>
			...
			<xsl:if test="$failureCount > 0">
				...
				<xsl:for-each select="/testResults/*[@lb = current()/@lb][attribute::s='false']">
					<tr>
						...
						<!--  <td class="data"><span class="oTime"><xsl:value-of select="@ts" /></span></td>-->
						<td class="data"><span class="patch_timestamp"><xsl:value-of select="@ts"/></span></td>
						<td>Request Failed</td>
						<xsl:if test="$showData = 'y'">
							<td style="word-wrap:break-word;" width="30%"><xsl:value-of select="@rc | @rs" /> - <xsl:value-of select="@rm" /><br/></td>
						</xsl:if>
					</tr>
				</xsl:for-each>
